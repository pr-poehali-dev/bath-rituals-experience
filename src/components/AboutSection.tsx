...
const values = [
  { icon: '🏆', title: 'Опытные специалисты', desc: 'Каждый мастер — сертифицированный профессионал с практическим опытом от 5 лет' },
  { icon: '🌿', title: 'Качественная косметика', desc: 'Работаем только с профессиональными средствами ведущих мировых брендов' },
  { icon: '❤️', title: 'Индивидуальный подход', desc: 'Программа каждой процедуры подбирается под ваши потребности и пожелания' },
];
...
export default function AboutSection() {
  ...
  return (
    <section id="about" className="relative py-28 overflow-hidden" style={{ background: 'var(--dark)' }}>
      <div className="absolute top-0 right-0 font-cormorant select-none pointer-events-none" style={{ fontSize: 'clamp(60px,15vw,200px)', color: 'rgba(212,168,87,0.03)', lineHeight: 0.9, fontStyle: 'italic', transform: 'translateX(5%)' }}>
        СПА
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10" ref={ref}>
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          {/* Visual */}
          <div style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateX(0)' : 'translateX(-50px)', transition: 'all 1s ease' }}>
            <div className="relative aspect-[3/4] rounded-sm overflow-hidden" style={{ background: 'var(--dark-3)', border: '1px solid rgba(212,168,87,0.1)' }}>
          ...
          {/* Text */}
          <div style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateX(0)' : 'translateX(50px)', transition: 'all 1s ease 0.2s' }}>
            ...
            <h2 className="font-cormorant text-5xl md:text-6xl font-light text-cream leading-[1.1] mb-8">
              Место, где<br />
              <em className="text-gold not-italic">тело отдыхает,</em><br />
              а кожа сияет
            </h2>
            ...
            <div className="flex flex-col gap-5">
              {values.map((v, i) => (
                <div
                  ...
                  style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateX(0)' : 'translateX(20px)', transition: `all 0.7s ease ${0.4 + i * 0.15}s` }}
                >
                  <div className="w-10 h-10 rounded-sm flex items-center justify-center flex-shrink-0 text-lg" style={{ background: 'rgba(212,168,87,0.1)', border: '1px solid rgba(212,168,87,0.2)' }}>
                    {v.icon}
                  </div>
                  <div>
